...
class Alessandro(Player):
    ...
    def strategy(self, opponent: Player) -> Action:
        ...
        action = None
        if 0 == len(opponent.history):
            what_did_they_do = None
        else:
            what_did_they_do = opponent.history[int(math.floor(len(opponent.history)/2))]
        
        if what_did_they_do == "C":
            action = self.defect(opponent)
        elif what_did_they_do == "D":            
            action = self.cooperate(opponent)
        else:
            action = self.cooperate(opponent)
        return action
